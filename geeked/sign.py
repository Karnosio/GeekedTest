...
class LotParser:
    def __init__(self):
        self.mapping = {"(n[11:14])+.+(n[12:14]+n[6:8])":'n[21:28]'}
        self.lot = []
        self.lot_res = []
...
lotParser = (
    LotParser()
)  # doesn't need to calculate the lot and lot_res every time, so were gonna cache it
...
function encrypt_asymmetric_2(input, key) {
    ...
    def encrypt_w(raw_input, pt) -> str:
        ...
        if pt == "1":
            ...
            raise NotImplementedError(
                "This type of encryption is not implemented yet. Create an issue"
            )
    ...
    @staticmethod
    def generate_pow(
        lot_number_pow, captcha_id_pow, hash_func, hash_version, bits, date, empty
    ) -> dict:
        ...
        while True:
            ...
            if bit_remainder == 0:
                if hashed_value and hashed_value.startswith(prefix):
                ...
                if hashed_value and hashed_value.startswith(prefix):
                    ...
                    if threshold is not None and int(hashed_value[length], 16) <= threshold:
                        return {"pow_msg": pow_string + h, "pow_sign": hashed_value}

    @staticmethod
    def generate_dummy_w(data: dict, captcha_id: str) -> str:
    ...
    def generate_w(data: dict, captcha_id: str, risk_type: str):
        ...
        if risk_type in ("ai", "invisible"):
            ...
            left = SlideSolver(
                requests.get(
                    f"https://static.geetest.com/{data['slice']}", timeout=10
                ).content,
                requests.get(
                    f"https://static.geetest.com/{data['bg']}", timeout=10
                ).content,
            ).find_puzzle_piece_position() + random.uniform(0, 0.5)
            base |= {
                "passtime": random.randint(600, 1200),  # time in ms it took to solve
                "setLeft": left,
                "userresponse": left / 1.0059466666666665
                + 2,  # 1.0059466666666665 = .8876 * 340 / 300
            }
        elif risk_type in ("winlinze", "gobang"):
            base |= {"userresponse": GobangSolver(data["ques"]).find_four_in_line()}
            ...
            raise NotImplementedError(
                f"This type ({risk_type}) of captcha is not implemented yet."
            )
